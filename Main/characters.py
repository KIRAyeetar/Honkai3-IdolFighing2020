...
class Judgment(object):
    ...
    def act_dealing(self, act_list, oneself, target):
        for index in act_list:
            if index[0] == 'TARGET':
                # 这里偷懒了导致鲁棒性很差，可以优化
                if index[1] == 'HEALTH': target.health += index[2]*index[3]*oneself.damage_rate
                if index[1] == 'STOP': target.stop_count += index[2]*index[3]
                if index[1] == 'STOP_SKILL': target.stop_skill_count += index[2]*index[3]
            elif index[0] == 'SELF':
                if index[1] == 'HEALTH': oneself.health += index[2]*index[3]
                if index[1] == 'STOP': oneself.stop_count += index[2]*index[3]
    ...
    def game_begin(self):
        ...
        while self.over_judge():
            ...
            if self.char_a.stop_count==0:
                ...
                self.char_a.stop_count = (abs(self.char_a.stop_count-1)+self.char_a.stop_count-1)/2
            self.char_a.print_attr(health_only=1)
            self.char_b.print_attr(health_only=1)

            # 处理char_b事件
            if self.char_b.stop_count == 0:
                if self.char_b.stop_skill_count == 0:
                    act_list = self.char_b.round_fight(self.char_a, round_count)
                else:
                    act_list = [['TARGET', 'HEALTH', self.char_b.gen_attack(self.char_a)[0], -1]]
                ...
                self.char_b.stop_count = (abs(self.char_b.stop_count-1)+self.char_b.stop_count-1)/2
            self.char_a.print_attr(health_only=1)
            self.char_b.print_attr(health_only=1)
...
class Character(object):
    ...
    def print_attr(self, health_only):
        if health_only==1:
            ...
            print("name: %s\nhealth: %d\nattack: %d\ndefense: %d\nspeed: %d" % (self.name, self.health, self.attack, self.defense, self.speed))
...
class Kiana(Character):
    def round_fight(self, char_target, round_count):
        ...
        if round_count % 2 == 0:
            ...
            if self.gen_attack(char_target)[1] == 1:
                hit_num = self.gen_attack(char_target)[0]
            else:
                hit_num = 0

        # 记录本次行为数据并返回
        act_list += [['TARGET', 'HEALTH', hit_num, -1]]
        act_list += [['SELF', 'STOP', stop_round_count, 1]]
        return act_list
...
class Mei(Character):
    def round_fight(self, char_target, round_count):
        ...
        # 每两回合触发一次技能,额外造成5次3点元素伤害;否则普攻，且有30%概率麻痹对方一回合
        hit_num = 0
        stop_round_count = 0
        if round_count % 2 == 0:
            # 是否被抵抗
        ...
        # 记录本次行为数据并返回
        act_list += [['TARGET', 'STOP', stop_round_count, 1]]
        act_list += [['TARGET', 'HEALTH', hit_num, -1]]
        return act_list
...
class Bronya(Character):
    def round_fight(self, char_target, round_count):
        ...
        # 每三回合触发一次技能,造成1-100伤害;否则普攻，且25%概率造成4*12伤害
        if round_count % 3 == 0:
            # 是否被抵抗
            flag = random.uniform(0, 1)
            if flag<char_target.skill_resist_rate:
            ...
            if self.gen_attack(char_target)[1] == 1:
                hit_num_1 = self.gen_attack(char_target)[0]
                hit_num_2 = 0
                flag = random.uniform(0, 1)
                if flag <= 0.25:
                    hit_num_2 += 4 * (abs(12 - char_target.defense) + 12 - char_target.defense) / 2
            else:
                hit_num_1 = 0
                hit_num_2 = 0
        ...
        # 记录本次行为数据并返回
        act_list += [['TARGET', 'HEALTH', hit_num_1, -1]]
        act_list += [['TARGET', 'HEALTH', hit_num_2, -1]]
        return act_list
...
class Himeko(Character):
    def round_fight(self, char_target, round_count):
        ...
        if char_target.if_single == 1:
            ...
            act_list += [['TARGET', 'HEALTH', hit_num*2, -1]]
        return act_list
    ...
    def round_fight(self, char_target, round_count):
        ...
        if round_count % 4 == 0:
            ...
            if flag<char_target.skill_resist_rate:
                ...
                stop_skill_count = 2
                # 偷懒的判断方法，可以优化
                if char_target.damage_rate==1:
                    char_target.damage_rate *= 0.6
        else:
            if self.gen_attack(char_target)[1] == 1:
                hit_num = self.gen_attack(char_target)[0]
                flag = random.uniform(0, 1)
        ...
        # 记录本次行为数据并返回
        act_list += [['TARGET', 'HEALTH', hit_num, -1]]
        act_list += [['TARGET', 'HEALTH', recover_num, 1]]
...
class Selee(Character):
    ...
    def round_fight(self, char_target, round_count):
        ...
        # 记录本次行为数据并返回
        act_list += [['TARGET', 'HEALTH', hit_num, -1]]
        act_list += [['SELF', 'HEALTH', recover_num, 1]]
        return act_list
    ...
    def round_fight(self, char_target, round_count):
        ...
        if round_count % 3 == 0:
            ...
            if flag < char_target.skill_resist_rate:
                ...
                hit_num = 7*(abs(16-char_target.defense)+16-char_target.defense)/2
        else:
            if self.gen_attack(char_target)[1] == 1:
                hit_num = self.gen_attack(char_target)[0]
            else:
    ...
    def round_fight(self, char_target, round_count):
        ...
        if round_count % 3 == 0:
            ...
            if flag < char_target.skill_resist_rate:
                ...
                hit_num = 5*(abs(16-char_target.defense)+16-char_target.defense)/2
        else:
            if self.gen_attack(char_target)[1] == 1:
                hit_num = self.gen_attack(char_target)[0]
                flag = random.uniform(0, 1)
                if flag <= 0.3:
                    if char_target.defense<=5:
                        char_target.defense = 0
                    else:
